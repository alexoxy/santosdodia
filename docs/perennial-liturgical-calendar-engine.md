...
The current reconciliation therefore contains 52 `temporale` rows, 11 `movable-or-transfer` rows and 68 exact fixed Sanctorale rows: 129 source-bound days and 236 explicit unresolved days. Canonical migration coverage is 131/389 because 20 January and 23 April each contain two separately reviewed fixed occurrences; 258 legacy occurrences remain outside the canonical shadow. The 19 suppressed weekday candidates and the three replacement rows at transfer origins are evidence, not additional coverage. `publicationAllowed` and migration promotion both remain `false`; neither the public read path nor D1 production changes before complete semantic equivalence.
