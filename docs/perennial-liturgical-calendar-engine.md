...
The current reconciliation therefore contains 111 `temporale` rows, 11 `movable-or-transfer` rows and 80 exact fixed Sanctorale rows: 198 source-bound days and 167 explicit unresolved days. Canonical migration coverage is 202/389 because 20 January, 3 February, 23 April and 28 April each contain two separately reviewed fixed occurrences; 187 legacy occurrences remain outside the canonical shadow. The perennial Roman sequence for 17–24 December is represented by eight fixed-date TemporalRules: six exact Portugal 2026 rows are source-bound, the 20 December candidate yields to the fourth Sunday of Advent, and the Portugal 24 December morning form remains under jurisdiction review. Suppressed candidates and replacement rows at transfer origins are evidence, not additional coverage. `publicationAllowed` and migration promotion both remain `false`; neither the public read path nor D1 production changes before complete semantic equivalence.
