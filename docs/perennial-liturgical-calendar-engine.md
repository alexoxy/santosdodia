...
The current reconciliation therefore contains 105 `temporale` rows, 11 `movable-or-transfer` rows and 80 exact fixed Sanctorale rows: 192 source-bound days and 173 explicit unresolved days. Canonical migration coverage is 196/389 because 20 January, 3 February, 23 April and 28 April each contain two separately reviewed fixed occurrences; 193 legacy occurrences remain outside the canonical shadow. The 26 suppressed family candidates and the three replacement rows at transfer origins are evidence, not additional coverage. `publicationAllowed` and migration promotion both remain `false`; neither the public read path nor D1 production changes before complete semantic equivalence.
