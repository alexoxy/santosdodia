...
All 52 Temporale rows were already part of the 65/389 migration-shadow total, so connecting the 47 family members to the ledger does not increase migration coverage. It aligns semantic reconciliation with the complete current migration subset: 52 Temporale rows plus 13 fixed Sanctorale rows, or 65 source-bound days, reducing the explicit unresolved partition from 347 to 300. The 19 suppressed weekday candidates do not count as coverage. `publicationAllowed` remains `false`; neither the migration gate nor the public read path changes.
