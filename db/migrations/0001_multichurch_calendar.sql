...
CREATE TABLE IF NOT EXISTS calendar_import_runs (
  ...
  dropbox_manifest_path TEXT NOT NULL,
  manifest_sha256 TEXT NOT NULL,
  status TEXT NOT NULL CHECK (status IN ('provisional','validated','rejected','promoted')),
...
CREATE TABLE IF NOT EXISTS calendar_sources (
  ...
  name TEXT NOT NULL,
  url TEXT NOT NULL,
  authority TEXT NOT NULL CHECK (authority IN ('official-church','official-jurisdiction','official-local-church','reference-engine','reference-directory','osint-corroboration')),
  usage_policy TEXT NOT NULL,
  copyright_policy TEXT,
  active INTEGER NOT NULL DEFAULT 1 CHECK (active IN (0,1)),
  UNIQUE(church_id, jurisdiction_id, url)
);
