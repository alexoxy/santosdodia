...
import { getMonthlyFeasts } from '../../data/feasts';
...
export default function CalendarioPage() {
  ...
  function prevMonth() {
    const m = new Date(Date.UTC(year, month, 1));
    m.setUTCMonth(m.getUTCMonth() - 1);
    setYear(m.getUTCFullYear());
    setMonth(m.getUTCMonth());
  }
  ...
  const monthName = new Intl.DateTimeFormat('pt', { month: 'long', timeZone: 'UTC' }).format(new Date(Date.UTC(year, month, 1)));
      ...
      <div className="card">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 6 }}>
          {['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom'].map((d) => (
            <div key={d} style={{ fontWeight: 600, color: 'var(--ink)', opacity: 0.8 }}>{d}</div>
          ))}
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  ...
                  {isToday ? <span style={{ color: 'var(--pine)', fontSize: 12 }}>hoje</span> : null}
                </div>
