import { readSaintTimeline } from '../../../../../lib/saint-navigation-d1';
...
export async function GET(request: Request) {
  try {
    ...
    const data = await readSaintTimeline(context.runtime.database, {
      locale: params.get('locale') ?? 'pt',
      century: optionalNumber(params.get('century')),
      fromYear: optionalNumber(params.get('fromYear')),
      toYear: optionalNumber(params.get('toYear')),
