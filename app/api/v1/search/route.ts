...
import { mergeObservances,parseCategory,parseTradition,traditionLabel } from '../../../../data/observances';
...
export async function GET(request:NextRequest){
 ...
 const curated=getPublicAllObservances(year,locale,filters),imported=live?await getChurchObservances(year,locale,filters):emptyImport,merged=mergeObservances(curated,imported.data);
 ...
 return Response.json({data,meta:{query:q,locale,year,count:data.length,withheldForTranslation:merged.length-localized.length,filters,live,sourceHealth:imported.sourceHealth,publication:imported.publication}},{headers:{'Cache-Control':'public, s-maxage=600, stale-while-revalidate=3600','Access-Control-Allow-Origin':'*'}})
}
