...
export async function GET(request:NextRequest,context:{params:Promise<{feed:string}>}){
 ...
 const groups=await Promise.all(years.map(async year=>{const curated=getAllObservances(year,locale,filters),imported=await getLiveObservances(year,'en',filters);return mergeObservances(curated,imported.data)}));
