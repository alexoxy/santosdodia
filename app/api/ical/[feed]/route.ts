...
import { availableFeeds, getAllObservances, mergeObservances, parseCategory, parseTradition, traditionLabel, type Tradition } from '../../../../data/observances';
...
function feedTradition(value: string): Tradition | undefined { if (Object.prototype.hasOwnProperty.call(availableFeeds, value)) return availableFeeds[value as keyof typeof availableFeeds] as Tradition | undefined; return parseTradition(value); }
...
export async function GET(request: NextRequest, context: { params: Promise<{ feed: string }> }) {
  ...
  const explicitYear = query.has('year') ? Number(query.get('year')) : undefined, currentYear = new Date().getUTCFullYear(), years = explicitYear ? [explicitYear] : [currentYear, currentYear + 1];
  const tradition = feedSelection ?? parseTradition(query.get('tradition')), category = parseCategory(query.get('category')), country = query.get('country') ?? undefined, filters = { tradition, category, country };
  const groups = await Promise.all(years.map(async year => { const curated = getAllObservances(year, locale, filters), imported = await getLiveObservances(year, locale, filters); return mergeObservances(curated, imported.data); }));
  const items = groups.flat(), calendarTitle = tradition ? `Santos do Dia — ${traditionLabel(copy, tradition)}` : `Santos do Dia — ${copy.calendarTitle}`, stamp = new Date().toISOString().replace(/[-:]/g, '').replace(/\.\d{3}Z$/, 'Z');
  ...
  for (const item of items) {
    ...
    const traditions = item.traditions.map(value => traditionLabel(copy, value)).join(', '), patronages = displayPatronages(item.patronages, locale);
  ...
  return new Response(`${lines.join('\r\n')}\r\n`, { headers: { 'Content-Type': 'text/calendar; charset=utf-8', 'Content-Disposition': `inline; filename="santos-do-dia-${feed}.ics"`, 'Cache-Control': 'public, s-maxage=3600, stale-while-revalidate=86400', 'Access-Control-Allow-Origin': '*' } });
}
