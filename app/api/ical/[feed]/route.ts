...
const escapeIcs = (value: string | undefined) =>
    String(value ?? "")
      ...
      .replaceAll(",", "\\,")
      .replaceAll(";", "\\;"),
  compact = (value: string) => value.replaceAll("-", "");
...
function feedTradition(value: string): Tradition | undefined {
  if (Object.prototype.hasOwnProperty.call(availableFeeds, value))
    return availableFeeds[value as keyof typeof availableFeeds] as
      | Tradition
      | undefined;
...
export async function GET(
  ...
  const { feed: raw } = await context.params,
    ...
    all = feed === "all",
    feedSelection = feedTradition(feed);
  if (!all && !feedSelection)
    return new Response("Calendar feed not found.", { status: 404 });
  const query = request.nextUrl.searchParams,
    locale = normalizeLocale(
      query.get("locale") ?? request.headers.get("accept-language"),
    ),
    copy = ui[locale],
    explicitYear = query.has("year") ? Number(query.get("year")) : undefined,
    currentYear = new Date().getUTCFullYear(),
    years = explicitYear ? [explicitYear] : [currentYear, currentYear + 1],
    tradition = feedSelection ?? parseTradition(query.get("tradition")),
    category = parseCategory(query.get("category")),
    country = query.get("country") ?? undefined,
    filters = { tradition, category, country };
  const groups = years.map((year) =>
    getPublicAllObservances(year, locale, filters),
  );
  const items = groups.flat(),
    calendarTitle = tradition
      ? `Santos do Dia — ${traditionLabel(copy, tradition)}`
      : `Santos do Dia — ${copy.calendarTitle}`,
    stamp = new Date()
      .toISOString()
      .replace(/[-:]/g, "")
      .replace(/\.\d{3}Z$/, "Z"),
  ...
  for (const item of items) {
    ...
    lines.push(
      ...
      `CATEGORIES:${escapeIcs(copy[item.category])},${escapeIcs(traditions)}`,
