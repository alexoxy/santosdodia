...
export default function DiscoveryTopicView({kind,slug}:{kind:DiscoveryKind;slug:string}){
 ...
 const items=getObservancesForTopic(topic,year,locale),eyebrow=kind==='place'?feature.byPlace:kind==='profession'?feature.byProfession:feature.associatedWith;
 return <div className="page-stack">
  <section className="page-hero compact-hero discovery-hero"><div><span className="eyebrow">{eyebrow}</span><h1>{topicLabel(topic,locale)}</h1><p>{topicDescription(topic,locale)}</p></div><div className="hero-symbol">✦</div></section>
  ...
  <section className="profile-grid">{items.map(item=>{const name=displayObservanceName(item.names,locale,item.name),patronages=displayPatronages(item.patronages,locale),related=topicsForObservance(item.id).filter(value=>value.slug!==topic.slug).slice(0,3);return <article className="saint-preview" key={item.id}><div className="saint-preview-date"><strong>{item.day}</strong><span>{new Intl.DateTimeFormat(locale,{month:'short',timeZone:'UTC'}).format(new Date(`${item.dateISO}T00:00:00Z`))}</span></div><div><div className="tag-row">{item.traditions.map(value=><span key={value}>{traditionLabel(copy,value)}</span>)}{patronages.slice(0,2).map(value=><span key={value}>{value}</span>)}</div><h2>{name}</h2>{item.summary?<p>{item.summary}</p>:null}<div className="saint-preview-links"><Link className="btn btn-primary" href={`/saint/${item.id}`}>{feature.openProfile}</Link><Link className="text-link" href={`/day/${item.dateISO}`}>{feature.openDay} →</Link></div>{related.length?<div className="related-mini">{related.map(value=><Link href={value.kind==='place'?`/place/${value.slug}`:`/patronage/${value.slug}`} key={value.slug}>{topicLabel(value,locale)}</Link>)}</div>:null}</div></article>})}</section>
