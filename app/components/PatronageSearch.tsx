...
export default function PatronageSearch({compact=false}:{compact?:boolean}){
 const{locale}=useLanguage();const copy=getFeatureCopy(locale);const[q,setQuery]=useState('');
 function submit(event:FormEvent){event.preventDefault();const value=q.trim(),destination=value?`/explore?q=${encodeURIComponent(value)}`:'/explore';window.location.assign(destination)}
 ...
 return <section className={`patron-search${compact?' patron-search-compact':''}`}>
  ...
  <form className="patron-search-form" onSubmit={submit} role="search"><label className="sr-only" htmlFor={compact?'patron-search-compact':'patron-search'}>{copy.findPlaceholder}</label><span aria-hidden="true">⌕</span><input id={compact?'patron-search-compact':'patron-search'} value={q} onChange={event=>setQuery(event.target.value)} placeholder={copy.findPlaceholder}/><button className="btn btn-primary" type="submit">{copy.findButton}</button></form>
