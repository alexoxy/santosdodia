...
export default function CopyrightSources(){
 ...
 const biographySources=[...new Map(SAINT_BIOGRAPHIES.flatMap(item=>item.sources).map(source=>[source.url,source])).values()];
 const sections=[
  ...
  [feature.copyrightData,'Calendar dates, names, patronages, historical summaries and jurisdictional associations are attributed to the source records below. A source link is evidence of provenance, not permission to reproduce an entire third-party publication.'],
  ...
  [feature.copyrightEmbeds,'Live video and recordings remain hosted by the originating institution or platform. Santos do Dia links to the official external page and does not claim ownership of the broadcast.'],
 ...
 return <div className="page-stack copyright-page">
  ...
  <section className="source-list-section"><div className="section-heading"><div><span className="eyebrow">Biography · editorial history · OSINT</span><h2>Biography sources</h2></div></div><div className="source-list">{biographySources.map(source=><article className="source-card" key={source.url}><div><span className="source-kind">{feature.officialSource}</span><h3>{source.name}</h3><div className="tag-row"><span>{source.publisher}</span><span>{source.language.toUpperCase()}</span></div></div><a className="text-link" href={source.url} target="_blank" rel="noreferrer">{feature.officialSource} ↗</a></article>)}<article className="source-card"><div><span className="source-kind">OSINT fallback</span><h3>Wikipedia · Wikimedia REST API</h3><p>When no editorial biography exists in the repository, the saint profile may display a localized Wikipedia summary. The original article is linked beside the text and remains available under its stated CC BY-SA licence.</p><div className="tag-row"><span>CC BY-SA</span><span>Multilingual</span><span>Cached by Vercel</span></div></div><a className="text-link" href="https://www.mediawiki.org/wiki/Wikimedia_REST_API" target="_blank" rel="noreferrer">API documentation ↗</a></article></div></section>
