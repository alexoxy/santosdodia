...
export default function SaintProfile({id}:{id:string}){
 const{locale,copy,timeZone}=useLanguage();const feature=getFeatureCopy(locale);const year=yearInTimeZone(timeZone);const item=getObservanceById(id,year,locale);
 ...
 const name=displayObservanceName(item.names,locale,item.name),patronages=displayPatronages(item.patronages,locale),topics=topicsForObservance(item.id),sources=item.sourceIds.map(sourceId=>SOURCE_CATALOG.find(source=>source.id===sourceId)).filter(Boolean),biography=getSaintBiography(item.id,locale),historyCopy=biographyUi[locale],summary=localizedSummary(item,locale);
