...
import { biographyUi,getSaintBiography,getSaintBiographyRecord } from '../../data/saint-biographies';
...
export default function SaintProfile({id,runtimeItem}:{id:string;runtimeItem?:Observance}){
 ...
 const biographyRecord=curatedItem?getSaintBiographyRecord(item.id):undefined,biography=curatedItem?getSaintBiography(item.id,locale):undefined,editorialReady=Boolean(biographyRecord&&isSaintBiographyIndexable(biographyRecord,locale));
 const name=displayObservanceName(item.names,locale,item.name),patronages=displayPatronages(item.patronages,locale),topics=curatedItem?topicsForObservance(item.id):[],historyCopy=biographyUi[locale],summary=localizedSummary(item,locale),scope=displayObservanceScope(item,locale,country),evidence=curatedItem?claimEvidenceFor(item.id):[],evidenceCopy=claimEvidenceUi[locale];
