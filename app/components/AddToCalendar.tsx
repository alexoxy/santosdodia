...
function downloadLabel(locale: Locale, fallback: string) {
  if (locale === 'pt') return 'Descarregar ficheiro de calendário';
  if (locale === 'es') return 'Descargar archivo de calendario';
  if (locale === 'fr') return 'Télécharger le fichier calendrier';
  if (locale === 'it') return 'Scarica file calendario';
  if (locale === 'en') return 'Download calendar file';
  return fallback;
}
...
export default function AddToCalendar({ feedPath, title = 'Santos do Dia', dateISO }: { feedPath: string; title?: string; dateISO?: string }) {
  ...
  return <div className="button-row"><a className="btn btn-primary" href={url}>{downloadLabel(locale,copy.downloadIcs)}</a><a className="btn btn-secondary" href={webcal}>{copy.apple}</a><a className="btn btn-secondary" href={google} target="_blank" rel="noreferrer">{copy.google}</a><a className="btn btn-secondary" href={outlook} target="_blank" rel="noreferrer">{copy.outlook}</a></div>;
}
