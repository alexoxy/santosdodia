...
export default function CalendarExplorer() {
  const {
    ...
    todayISO = useMemo(() => dateISOInTimeZone(timeZone), [timeZone]),
    todayYear = Number(todayISO.slice(0, 4)),
    todayMonth = Number(todayISO.slice(5, 7)) - 1,
    previousDayISO = useMemo(() => shiftISO(todayISO, -1), [todayISO]),
    nextDayISO = useMemo(() => shiftISO(todayISO, 1), [todayISO]),
    relativeDay = useMemo(() => {
    ...
    [year, setYear] = useState(todayYear),
  ...
  function detailHref(item: Observance, date: string) {
    return substantiveDetailHref(item, year, locale, date);
  }
  ...
  if (!contextReady) {
    return (
      <section className="calendar-card calendar-context-loading" aria-live="polite">
        {copy.loading}
      </section>
    );
  }
