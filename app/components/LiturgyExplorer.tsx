...
function colourValue(value:string|string[]|undefined){const raw=Array.isArray(value)?value[0]:value;if(!raw)return'#d8b35c';const normalized=raw.toLowerCase();const map:Record<string,string>={white:'#f4efe2',red:'#a84444',green:'#3f7754',purple:'#71558c',violet:'#71558c',rose:'#c87991',pink:'#c87991',black:'#24272b',gold:'#cba54d'};return map[normalized]??raw}
function textValue(value:unknown):string{if(typeof value==='string')return value;if(typeof value==='number'||typeof value==='boolean')return String(value);if(Array.isArray(value))return value.map(textValue).filter(Boolean).join(' · ');if(value&&typeof value==='object')return Object.entries(value as Record<string,unknown>).map(([key,item])=>`${key}: ${textValue(item)}`).join(' · ');return''}
function readingRows(value:unknown):string[]{if(!value)return[];if(Array.isArray(value))return value.flatMap(readingRows);if(typeof value==='string'||typeof value==='number')return[String(value)];if(typeof value==='object')return Object.entries(value as Record<string,unknown>).map(([key,item])=>`${key}: ${textValue(item)}`);return[]}
...
function EventCard({event,copy}:{event:LitcalEvent;copy:(typeof liturgyUi)[keyof typeof liturgyUi]}){
 ...
 const details=[
  [copy.colour,colours],[copy.season,event.season],[copy.common,commons],
  [copy.sundayCycle,event.sundayCycle],[copy.weekdayCycle,event.weekdayCycle],[copy.psalterWeek,event.psalterWeek]
 ].filter(([,value])=>value!==undefined&&value!=='');
 return <article className={styles.event}>
  <header className={styles.eventHeader}><div>{event.grade?<span className={styles.grade}>{event.grade}</span>:null}<h2>{event.name}</h2></div><span className={styles.colour} style={{background:colourValue(event.colour)}} title={colours}/></header>
...
export default function LiturgyExplorer(){
 ...
 const query=useMemo(()=>{const p=new URLSearchParams({date,locale,kind:calendar.kind});if(calendar.id)p.set('calendar',calendar.id);return p},[date,locale,calendar]);
 ...
 return <div className={styles.page}>
  <section className={styles.hero}><span className={styles.eyebrow}>LitCal · General Roman Calendar</span><h1>{copy.title}</h1><p>{copy.intro}</p></section>
  <section className={styles.controls}>
   ...
   <label className={styles.field}><span>{copy.calendar}</span><select value={calendarValue(calendar)} onChange={event=>setCalendar(parseCalendar(event.target.value))}>{calendars.map(item=><option key={calendarValue(item)} value={calendarValue(item)}>{item.kind==='general'?copy.generalCalendar:item.label??item.id}</option>)}</select></label>
  ...
  {result?<>
   ...
   <section className={styles.event}><div className={styles.section}><h3>{copy.provenance}</h3><div className={styles.links}><a href={`/api/v1/liturgy?${query}`} target="_blank" rel="noreferrer">{copy.rawData}</a><a href="https://github.com/Liturgical-Calendar/LiturgicalCalendarAPI" target="_blank" rel="noreferrer">{copy.sourceCode}</a></div></div><details className={styles.section}><summary>{copy.rawData}</summary><pre className={styles.raw}>{JSON.stringify({settings:result.settings,metadata:result.metadata},null,2)}</pre></details></section>
  </>:null}
 </div>;
}
