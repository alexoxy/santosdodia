...
import { getObservancesForDate,isValidDateISO,traditionClass,traditionLabel,type Observance } from '../../data/observances';
...
export default function DayView({dateISO}:{dateISO:string}){
 ...
 return <div className="page-stack">
  ...
  <section className="day-list">{items.length?items.map(item=>{const patronages=displayPatronages(item.patronages,locale),profile=Boolean(getObservanceById(item.id,year,locale));return <article className="day-observance" key={item.id}><div className="day-observance-main"><div className={`tradition-emblem ${traditionClass(item.traditions[0])}`}>✦</div><div><div className="tag-row">{item.traditions.map(value=><span key={value}>{traditionLabel(copy,value)}</span>)}<span>{copy[item.category]}</span><span>{displayCalendarSystem(item.calendarSystem,locale)}</span><span>{validationStatusLabel(item.validationStatus,locale)}</span></div><h2>{displayObservanceName(item.names,locale,item.name)}</h2>{item.summaries?.[locale]?<p>{item.summaries[locale]}</p>:null}{patronages.length?<div className="patronage-line"><strong>{copy.patronage}:</strong> {patronages.join(' · ')}</div>:null}{profile?<Link className="text-link" href={`/saint/${item.id}`}>{feature.openProfile} →</Link>:null}</div></div><CandleButton observanceId={item.id} dateISO={dateISO}/></article>}) : <div className="empty-state large"><span>✦</span><p>{loading?copy.loading:copy.noObservances}</p></div>}</section>
