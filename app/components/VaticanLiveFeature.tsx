...
export default function VaticanLiveFeature(){
 const{locale}=useLanguage();const copy=getFeatureCopy(locale);const vatican=getVaticanLiveCopy(locale);const[enabled,setEnabled]=useState(false);
 return <section className="vatican-live-feature" aria-labelledby="vatican-live-title">
  <div className="vatican-live-copy"><span className="eyebrow">{vatican.eyebrow}</span><h2 id="vatican-live-title">{vatican.title}</h2><p>{vatican.intro}</p><div className="vatican-live-actions">{enabled?<a className="text-link" href={LIVE_URL} target="_blank" rel="noreferrer">{copy.openLive} →</a>:<button className="btn btn-primary" type="button" onClick={()=>setEnabled(true)}>{copy.openLive}</button>}<a className="btn btn-secondary" href={OFFICIAL_URL} target="_blank" rel="noreferrer">{copy.officialSource}</a><a className="text-link" href="/live">{vatican.otherLives} →</a></div></div>
  <div className="vatican-live-media">{enabled?<iframe src={`https://www.youtube-nocookie.com/embed/live_stream?channel=${CHANNEL_ID}&autoplay=0`} title={vatican.title} loading="lazy" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen/>:<button type="button" className="vatican-live-placeholder" onClick={()=>setEnabled(true)} aria-label={copy.openLive}><span aria-hidden="true">▶</span><strong>Vatican News</strong><small>{copy.liveFallback}</small></button>}</div>
  <style jsx>{`
