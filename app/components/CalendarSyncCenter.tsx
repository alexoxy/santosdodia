...
type SyncCopy = {
  ...
  refreshNote:string; calendarName:string; previewTitle:string; previewBody:string; previewAction:string;
};

const labels: Partial<Record<Locale,SyncCopy>> = {
  en:{eyebrow:'Subscribe once · keep it updated',title:'Sync the liturgical calendar',intro:'Build one persistent calendar feed from the same reviewed data that powers Today and the calendar. Choose a Church, region and category, then subscribe in your calendar app or use the equivalent JSON API.',liveTitle:'Live subscription',liveBody:'Best for normal use. The URL stays stable and serves a rolling five-year window: the previous civil year, the current year and the next three years. It advances automatically without requiring a new subscription.',snapshotTitle:'Annual snapshot',snapshotBody:'Download one fixed calendar year as an ICS file for archiving, import or offline use.',machineTitle:'Machine access',machineBody:'Use the equivalent JSON endpoint for apps, agents, websites and integrations. The same filters are preserved.',church:'Church',region:'Region',category:'Category',all:'All',global:'Global',year:'Year',subscriptionUrl:'Subscription URL',apiUrl:'JSON API URL',copy:'Copy',copied:'Copied',downloadYear:'Download annual ICS',openApi:'Open API guide',openJson:'Open JSON',refreshNote:'External calendar apps decide how often they refresh subscribed calendars. Santos do Dia publishes a six-hour refresh hint in the feed.',calendarName:'Santos do Dia liturgical calendar',previewTitle:'Reviewed preview · subscription not yet ready',previewBody:'This Church and region can be explored, but Santos do Dia does not yet claim a complete authority-verified calendar subscription for this context.',previewAction:'Explore reviewed records'},
  pt:{eyebrow:'Subscreva uma vez · mantenha atualizado',title:'Sincronizar o calendário litúrgico',intro:'Crie uma subscrição persistente a partir dos mesmos dados revistos que alimentam o Hoje e o calendário. Escolha Igreja, região e categoria e sincronize com a sua agenda ou use a API JSON equivalente.',liveTitle:'Subscrição contínua',liveBody:'É a opção recomendada. O endereço mantém-se estável e serve uma janela móvel de cinco anos: o ano civil anterior, o atual e os três seguintes. A janela avança automaticamente sem exigir nova subscrição.',snapshotTitle:'Snapshot anual',snapshotBody:'Descarregue um ano civil fixo em ICS para arquivo, importação ou utilização offline.',machineTitle:'Acesso por API',machineBody:'Use o endpoint JSON equivalente em aplicações, agentes, sites e integrações. Os mesmos filtros são preservados.',church:'Igreja',region:'Região',category:'Categoria',all:'Todas',global:'Global',year:'Ano',subscriptionUrl:'URL de subscrição',apiUrl:'URL da API JSON',copy:'Copiar',copied:'Copiado',downloadYear:'Descarregar ICS anual',openApi:'Abrir guia da API',openJson:'Abrir JSON',refreshNote:'Cada aplicação externa decide a frequência com que atualiza calendários subscritos. O Santos do Dia publica no feed uma recomendação de atualização de seis horas.',calendarName:'Calendário litúrgico Santos do Dia',previewTitle:'Pré-visualização revista · subscrição ainda não pronta',previewBody:'Pode explorar esta Igreja e região, mas o Santos do Dia ainda não declara uma subscrição completa e verificada por fontes competentes para este contexto.',previewAction:'Explorar registos revistos'},
  es:{eyebrow:'Suscríbete una vez · mantenlo actualizado',title:'Sincronizar el calendario litúrgico',intro:'Crea una suscripción persistente a partir de los mismos datos revisados que alimentan Hoy y el calendario. Elige Iglesia, región y categoría y sincroniza con tu agenda o utiliza la API JSON equivalente.',liveTitle:'Suscripción continua',liveBody:'Es la opción recomendada. La dirección permanece estable y sirve una ventana móvil de cinco años: el año civil anterior, el actual y los tres siguientes. La ventana avanza automáticamente sin exigir una nueva suscripción.',snapshotTitle:'Instantánea anual',snapshotBody:'Descarga un año civil fijo en ICS para archivo, importación o uso sin conexión.',machineTitle:'Acceso API',machineBody:'Utiliza el endpoint JSON equivalente para aplicaciones, agentes, sitios e integraciones. Se conservan los mismos filtros.',church:'Iglesia',region:'Región',category:'Categoría',all:'Todas',global:'Global',year:'Año',subscriptionUrl:'URL de suscripción',apiUrl:'URL API JSON',copy:'Copiar',copied:'Copiado',downloadYear:'Descargar ICS anual',openApi:'Abrir guía de API',openJson:'Abrir JSON',refreshNote:'Cada aplicación externa decide la frecuencia de actualización. Santos do Dia publica una recomendación de seis horas en el feed.',calendarName:'Calendario litúrgico Santos do Dia',previewTitle:'Vista previa revisada · suscripción aún no lista',previewBody:'Puedes explorar esta Iglesia y región, pero Santos do Dia aún no declara una suscripción completa y verificada por fuentes competentes para este contexto.',previewAction:'Explorar registros revisados'},
  it:{eyebrow:'Iscriviti una volta · resta aggiornato',title:'Sincronizza il calendario liturgico',intro:'Crea un abbonamento persistente dagli stessi dati revisionati che alimentano Oggi e il calendario. Scegli Chiesa, regione e categoria e sincronizza con la tua agenda oppure usa l’API JSON equivalente.',liveTitle:'Abbonamento continuo',liveBody:'È l’opzione consigliata. L’indirizzo resta stabile e serve una finestra mobile di cinque anni: l’anno civile precedente, quello corrente e i tre successivi. La finestra avanza automaticamente senza richiedere un nuovo abbonamento.',snapshotTitle:'Snapshot annuale',snapshotBody:'Scarica un anno civile fisso in formato ICS per archivio, importazione o uso offline.',machineTitle:'Accesso API',machineBody:'Usa l’endpoint JSON equivalente per applicazioni, agenti, siti e integrazioni. Gli stessi filtri vengono mantenuti.',church:'Chiesa',region:'Regione',category:'Categoria',all:'Tutte',global:'Globale',year:'Anno',subscriptionUrl:'URL di abbonamento',apiUrl:'URL API JSON',copy:'Copia',copied:'Copiato',downloadYear:'Scarica ICS annuale',openApi:'Apri guida API',openJson:'Apri JSON',refreshNote:'Ogni app esterna decide la frequenza di aggiornamento. Santos do Dia pubblica nel feed una raccomandazione di sei ore.',calendarName:'Calendario liturgico Santos do Dia',previewTitle:'Anteprima revisionata · abbonamento non ancora pronto',previewBody:'Puoi esplorare questa Chiesa e regione, ma Santos do Dia non dichiara ancora un abbonamento completo e verificato da fonti competenti per questo contesto.',previewAction:'Esplora i dati revisionati'},
};
...
export default function CalendarSyncCenter(){
  ...
  const subscriptionReady = isPublicCalendarSubscriptionReady(selectedChurch,selectedCountry);
  ...
  return <div className="page-stack">
    ...
    <section className="filter-panel" aria-label={text.title}>
      ...
      <div className="filter-group"><label>{text.year}</label><select value={year} onChange={event=>setYear(Number(event.target.value))}>{rollingYears.map(value=><option key={value} value={value}>{value}</option>)}</select></div>
    </section>

    {subscriptionReady ? <section className="institutional-grid">
    ...
    </section> : <section className="institutional-grid"><article className="institutional-card"><span className="eyebrow">{text.previewTitle}</span><h2>{traditionLabel(copy,selectedChurch === 'all' ? 'roman-catholic' : selectedChurch)}</h2><p>{text.previewBody}</p><a className="btn btn-secondary" href="/calendar">{text.previewAction}</a></article></section>}
  </div>;
}
