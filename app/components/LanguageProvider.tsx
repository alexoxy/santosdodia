...
export default function LanguageProvider({initialLocale,initialCountry,initialChurch='roman-catholic',children}:{initialLocale:Locale;initialCountry?:string;initialChurch?:ChurchPreference;children:React.ReactNode}){
 ...
 useEffect(()=>{const saved=window.localStorage.getItem('sdd-locale');const next=saved?normalizeLocale(saved):normalizeLocale(navigator.languages?.[0]??navigator.language);setLocaleState(next);document.documentElement.lang=next;setTimeZone(normalizeTimeZone(Intl.DateTimeFormat().resolvedOptions().timeZone));const storedChurch=window.localStorage.getItem('sdd-tradition');if(storedChurch==='all')setChurchState('all');else{const parsed=parseTradition(storedChurch);if(parsed)setChurchState(parsed)}},[]);
