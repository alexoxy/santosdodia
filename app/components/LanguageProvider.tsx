...
import { normalizePublicLocale, type Locale, ui } from '../../lib/i18n';
...
export default function LanguageProvider({initialLocale,initialCountry,initialChurch='roman-catholic',initialTimeZone,children}:{initialLocale:Locale;initialCountry?:string;initialChurch?:ChurchPreference;initialTimeZone?:string;children:React.ReactNode}){
 ...
 function setLocale(next:Locale){const publicLocale=normalizePublicLocale(next);setLocaleState(publicLocale);window.localStorage.setItem('sdd-locale',publicLocale);document.cookie=`sdd-locale=${publicLocale}; path=/; max-age=31536000; samesite=lax`;document.documentElement.lang=publicLocale}
