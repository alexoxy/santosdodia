...
function mapped(value:string|undefined,locale:Locale,table:Record<string,Partial<Record<Locale,string>>>){if(!value)return value;const key=canonical(value);return table[key]?.[locale]??(locale==='en'?value:undefined)}
function localizeArray(value:string|string[]|undefined,locale:Locale,table:Record<string,Partial<Record<Locale,string>>>):string|string[]|undefined{return Array.isArray(value)?value.map(item=>mapped(item,locale,table)).filter((item):item is string=>Boolean(item)):mapped(value,locale,table)}
function names(name:string,locale:Locale):LocalizedText{return{en:name,[locale]:name} as LocalizedText}
function localizeEvent(event:LitcalEvent,locale:Locale):LitcalEvent|undefined{
 const name=displayObservanceName(names(event.name,locale),locale,event.name);if(!name)return;
 return{...event,name,grade:mapped(event.grade,locale,GRADES),colour:localizeArray(event.colour,locale,COLORS),season:mapped(event.season,locale,SEASONS),common:Array.isArray(event.common)?event.common.map(value=>displayObservanceName(names(value,locale),locale,value)).filter(Boolean):event.common?displayObservanceName(names(event.common,locale),locale,event.common)||undefined:undefined};
}
...
export function localizeLitcalDay(result:LitcalDayResult,locale:Locale):LitcalDayResult{
 ...
 return{...result,events:result.events.map(event=>localizeEvent(event,locale)).filter((event):event is LitcalEvent=>Boolean(event)),messages:sourceSupportsLocale?result.messages:[]};
}
