...
function validateRule(rule: RomanSanctoraleRule, knownScopes: Set<string>): void {
  ...
  for (const evidence of rule.evidence) {
    if (!evidence.publisher || !validHttpsUrl(evidence.url) || !Array.isArray(evidence.claimTypes) || evidence.claimTypes.length === 0) {
...
export function validateRomanSanctoraleInputs(
  ...
  for (const policy of policyDataset.policies) {
    ...
    if (!policy.jurisdictionId || policy.scopeOrderSemantics !== 'least-to-most-specific' || policy.overrideKey !== 'observanceId') {
      throw new RangeError(`Policy ${policy.id} has invalid inheritance semantics.`);
    }
    ...
    for (const scope of policy.inheritedScopeKeys) knownScopes.add(scope);
    if (!Array.isArray(policy.authorityDomains) || policy.authorityDomains.length === 0) throw new RangeError(`Policy ${policy.id} requires authority domains.`);
  }

  const ruleIds = new Set<string>();
  const observanceScope = new Set<string>();
  for (const rule of dataset.rules) {
    validateRule(rule, knownScopes);
