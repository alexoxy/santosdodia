...
export function normalizePublicLocale(value:string|null|undefined):Locale {
  const locale=normalizeLocale(value);
  return (PUBLIC_LOCALES as readonly string[]).includes(locale)?locale:'en';
}
export function localeFromAcceptLanguage(value:string|null|undefined):Locale {
  if(!value)return'en';
  for(const token of value.split(',')){
    const raw=token.split(';')[0]?.trim().toLowerCase().replace('_','-')??'';
    const code=raw.split('-')[0];
    const alias=code==='tl'||code==='ph'?'fil':code;
    if((PUBLIC_LOCALES as readonly string[]).includes(alias))return alias as Locale;
  }
  return'en';
}
...
const en={
 ...
 searchIntro:'Search by name, date, country, category or Christian tradition.',
 ...
 approvedData:'Approved repository data',lastChecked:'Last checked',validation:'Validation',verified:'Verified',
...
const es:UiCopy={
 ...
 searchIntro:'Busca por nombre, fecha, país, categoría o tradición cristiana.',
 ...
 approvedData:'Datos aprobados del repositorio',lastChecked:'Última comprobación',validation:'Validación',verified:'Verificado',
...
const pt:UiCopy={
 ...
 searchIntro:'Pesquise por nome, data, país, categoria ou tradição cristã.',
 ...
 approvedData:'Dados aprovados do repositório',lastChecked:'Última verificação',validation:'Validação',verified:'Verificado',
...
const fr:UiCopy={
 ...
 searchIntro:'Recherchez par nom, date, pays, catégorie ou tradition chrétienne.',
 ...
 approvedData:'Données approuvées du dépôt',lastChecked:'Dernière vérification',validation:'Validation',verified:'Vérifié',
...
const it:UiCopy={
 ...
 saintsToday:'Santi e ricorrenze di oggi',noObservances:'Non sono ancora disponibili ricorrenze verificate per questa data.',
 ...
 calendarIntro:'Consulta le ricorrenze verificate e crea un calendario per tradizione, data, regione o tema.',
 ...
 searchIntro:'Cerca per nome, data, Paese, categoria o tradizione cristiana.',
 searchPlaceholder:'Cerca Sant’Antonio, medici, Italia…',results:'risultati',noResults:'Nessuna ricorrenza corrispondente.',
 sourcesTitle:'Fonti e validazione',sourcesIntro:'Ogni record conserva la fonte, la tradizione, il sistema di calendario e lo stato di validazione.',
 candle:'Accendi una candela',candleLit:'Candela accesa',candleEmpty:'Oggi non è stata accesa alcuna candela su questo dispositivo.',
 candleCount:'su questo dispositivo',suggestedRegion:'Regione suggerita',beta:'Base dati in continuo aggiornamento',
 ...
 disclaimer:'Date e ricorrenze possono variare secondo la Chiesa, la giurisdizione, il rito e il sistema di calendario.',
 dateInvalid:'Data non valida',backCalendar:'Torna al calendario',observancesOn:'Ricorrenze del',
 feedAll:'Calendario combinato',feedCatholic:'Calendario cattolico romano',feedOrthodox:'Calendario ortodosso',
 ...
 approvedData:'Dati approvati',lastChecked:'Ultimo controllo',validation:'Validazione',verified:'Verificato',
 reviewRequired:'Revisione necessaria',patronage:'Patronato',calendarSystem:'Sistema di calendario',
 machineAccess:'Accesso per sistemi automatici',machineAccessIntro:'JSON strutturato, ICS e OpenAPI sono disponibili per motori di ricerca e agenti di IA.',
 ...
 ethiopianOrthodox:'Ortodossa etiopica Tewahedo',syriacOrthodox:'Ortodossa siriaca'
};

// Retained internally for source ingestion and language work. They are not
// exposed in the public selector until the full product-copy contract passes.
