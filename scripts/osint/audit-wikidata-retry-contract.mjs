...
for (const needle of [
  ...
  if (!adapter.includes(needle)) errors.push(`Wikidata adapter is missing retry/cursor contract: ${needle}`);
}
...
if (!adapter.includes("retryPolicy: { maxAttempts: retryAttempts")) errors.push('Run summary must preserve the retry policy used.');
