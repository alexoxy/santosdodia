...
function centuryLabel(century) {
  if (!Number.isInteger(century) || century === 0) return null;
  return century > 0 ? `${century}` : `${Math.abs(century)} BCE`;
}

function displayName(person, locale) {
  return person?.names?.[locale] || person?.names?.en || person?.canonicalName || person?.entityId || '';
}
...
function normalizedPerson(person, locale) {
  ...
  const century = centuryForYear(anchorYear);
  return {
    entityId: person.entityId,
    qid: person.qid ?? null,
    name: displayName(person, locale),
    names: person.names ?? {},
    birth: person.birth ?? null,
    death: person.death ?? null,
    anchorYear,
    century,
    centuryLabel: centuryLabel(century),
    traditions: unique(person.traditions ?? []).sort(),
    categories: unique(person.categories ?? []).sort(),
...
export function buildNavigationExports(input, { mode = 'staging', locale = 'pt' } = {}) {
  ...
  for (const source of people) {
    ...
    for (const place of source.places ?? []) {
      ...
      features.push({
        type: 'Feature',
        id: `${source.entityId}:${place.relationType}:${place.placeId ?? `${lat},${lon}`}`,
        ...
        properties: {
          ...
          placeName: place.currentName ?? place.name ?? null,
          historicalName: place.historicalName ?? null,
          countryCode: place.countryCode ?? null,
          century: person.century,
          anchorYear: person.anchorYear,
          confidence: Number.isFinite(place.confidence) ? place.confidence : null,
          sourceIds: unique(place.sourceIds ?? []).sort()
        }
      });
    }
  }
  ...
  const timelineItems = index
    .filter((person) => person.anchorYear !== null)
    .map((person) => ({
      entityId: person.entityId,
      qid: person.qid,
      name: person.name,
      ...
      anchorYear: person.anchorYear,
      century: person.century,
      centuryLabel: person.centuryLabel,
      traditions: person.traditions,
      categories: person.categories
    }))
    .sort((a, b) => a.anchorYear - b.anchorYear || a.name.localeCompare(b.name, locale));
  const byCentury = {};
  for (const item of timelineItems) {
    const key = String(item.century);
    byCentury[key] ??= [];
    byCentury[key].push(item.entityId);
  }

  const calendar = {};
  for (const source of people) {
    for (const observance of source.observances ?? []) {
      if (mode === 'public' && observance.publicationStatus !== 'published') continue;
      if (!Number.isInteger(observance.month) || !Number.isInteger(observance.day)) continue;
      const key = `${pad(observance.month)}-${pad(observance.day)}`;
      calendar[key] ??= [];
      calendar[key].push({
        observanceId: observance.id,
        entityId: source.entityId,
        name: personById.get(source.entityId)?.name ?? source.entityId,
        churchId: observance.churchId ?? null,
        jurisdictionId: observance.jurisdictionId ?? null,
        rankCode: observance.rankCode ?? null,
        validationStatus: observance.validationStatus ?? source.validationStatus,
        publicationStatus: observance.publicationStatus ?? source.publicationStatus
      });
    }
  }
  ...
  const outputs = {
    ...
    timeline: { schemaVersion: 1, items: timelineItems, byCentury },
    calendar: { schemaVersion: 1, days: Object.fromEntries(Object.entries(calendar).sort(([a], [b]) => a.localeCompare(b))) }
  };
  ...
  return {
    ...
    manifest: {
      ...
      calendarDayCount: Object.keys(calendar).length,
      hashes,
...
async function main() {
  ...
  const outputDir = path.resolve(argument('--output', 'staging/navigation-exports'));
  const mode = argument('--mode', 'staging');
  const locale = argument('--locale', 'pt');
  if (!argument('--input')) throw new Error('--input is required.');
  const input = JSON.parse(fs.readFileSync(inputPath, 'utf8'));
  const result = buildNavigationExports(input, { mode, locale });
  ...
  fs.writeFileSync(path.join(outputDir, 'saints-map.geojson'), stableJson(result.map), 'utf8');
  fs.writeFileSync(path.join(outputDir, 'saints-timeline.json'), stableJson(result.timeline), 'utf8');
  fs.writeFileSync(path.join(outputDir, 'daily-calendar.json'), stableJson(result.calendar), 'utf8');
  fs.writeFileSync(path.join(outputDir, 'manifest.json'), stableJson(result.manifest), 'utf8');
