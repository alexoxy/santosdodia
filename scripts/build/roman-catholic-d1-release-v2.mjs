...
function sourceUrl(sourceId) {
  ...
  if (sourceId === 'holy-see-calendar-authority') return 'https://www.vatican.va/';
  if (sourceId === 'santosdia-reviewed-calendar-localization') return 'https://www.santosdodia.com/';
  return '';
}
function occurrenceId(item) { return `rc-pt-v2-${item.dateISO}-${hash(item.sourceOccurrenceId).slice(0, 16)}`; }
...
export function buildPortugalD1ReleaseV2({ report, dropboxManifestPath, publicationStatus = 'publishable', generatedAt = new Date().toISOString() }) {
  ...
  if (report?.build !== 'roman-catholic-pt-overlay-v2' || report?.productReadiness?.stagingReady !== true || report?.productionWriteAllowed !== false || report?.productReadiness?.productionApproved !== false) throw new Error('Input is not a staging-ready Portugal overlay v2 build with production still gated.');
  if (report?.calendarCoverage?.occurrences !== 389 || report?.calendarCoverage?.coveredDays !== 365 || report?.productReadiness?.labelCount !== 1945) throw new Error('Portugal v2 release requires exactly 389 observances, 365 days and 1,945 labels.');
  for (const locale of PUBLIC_LOCALES) if (report?.localeCompleteness?.[locale]?.completeness !== 1) throw new Error(`Locale ${locale} is incomplete.`);
  ...
  const sources = [
    { id:'portugal-national-liturgy-secretariat', name:'Secretariado Nacional de Liturgia — Agenda Litúrgica', url:'https://www.liturgia.pt/agenda/', host:'www.liturgia.pt', authority:'official-jurisdiction', jurisdiction:"'pt'", usage:'Official Portugal occurrence dates, ranks and Portuguese labels after reviewed canonical binding.', copyright:'Structured calendar facts and short labels only.' },
    { id:'litcal-api', name:'LitCal API', url:'https://litcal.johnromanodorazio.com/api/v5', host:'litcal.johnromanodorazio.com', authority:'reference-engine', jurisdiction:'NULL', usage:'General Roman identity-linked English, French and Italian labels only in this Portugal v2 release.', copyright:'No substantial editorial text copied.' },
    { id:'romcal-general-roman-es', name:'Romcal General Roman Spanish locale', url:'https://github.com/romcal/romcal', host:'github.com', authority:'reference-directory', jurisdiction:'NULL', usage:'Spanish General Roman localization by event identity; never Portugal calendar authority.', copyright:'MIT-licensed localization data; attribution retained.' },
    { id:'holy-see-calendar-authority', name:'Holy See — General Roman Calendar authority', url:'https://www.vatican.va/', host:'www.vatican.va', authority:'official-universal', jurisdiction:'NULL', usage:'Higher-authority General Roman decrees used only when an operational mirror is stale or incomplete.', copyright:'Normative calendar facts and official short celebration names only.' },
    { id:'santosdia-reviewed-calendar-localization', name:'Santos do Dia reviewed calendar localization', url:'https://www.santosdodia.com/', host:'www.santosdodia.com', authority:'reviewed-editorial', jurisdiction:"'pt'", usage:'Human-reviewed localization for Portugal-specific canonical observances and explicit label-only identity bridges.', copyright:'Original reviewed localization produced for Santos do Dia.' },
  ];
  ...
  const usedOccurrenceIds = new Set(); let labelAssertions = 0; let snlAssertions = 0;
  for (const item of occurrences) {
    const dateISO = text(item.dateISO); const canonicalEventId = text(item.canonicalEventId);
    ...
    const id = occurrenceId(item); if (usedOccurrenceIds.has(id)) throw new Error(`Duplicate generated occurrence id ${id}.`); usedOccurrenceIds.add(id);
    ...
    const assertionHash = text(item.source.sourceRecordHash); const assertionId = hash(`${id}|portugal-national-liturgy-secretariat|${assertionHash}`);
    statements.push(`INSERT INTO calendar_occurrence_assertions (id,occurrence_id,source_id,asserted_date_iso,source_record_url,source_record_hash,observed_at,validation_status) VALUES (${sql(assertionId)},${sql(id)},'portugal-national-liturgy-secretariat',${sql(dateISO)},'https://www.liturgia.pt/agenda/agenda.ics',${sql(assertionHash)},${sql(generatedAt)},'cross-checked');`); snlAssertions += 1;
    for (const locale of PUBLIC_LOCALES) {
      const labelInfo = item.labels?.[locale]; const label = text(labelInfo?.label); const sourceId = text(labelInfo?.source); const translationStatus = text(labelInfo?.translationStatus);
      if (!label || !sourceId || !knownSourceIds.has(sourceId) || !['source','reviewed'].includes(translationStatus)) throw new Error(`${canonicalEventId} is missing validated ${locale} label provenance.`);
      const claimSourceUrl = text(labelInfo?.sourceUrl) || sourceUrl(sourceId); if (!claimSourceUrl) throw new Error(`${canonicalEventId} ${locale} label source URL is missing.`);
      ...
      statements.push(`INSERT INTO source_assertions (id,subject_type,subject_id,field,value_json,source_id,source_url,observed_at,content_hash,confidence) VALUES (${sql(claimId)},'calendar-occurrence',${sql(id)},${sql(`label.${locale}`)},${sql(JSON.stringify(label))},${sql(sourceId)},${sql(claimSourceUrl)},${sql(generatedAt)},${sql(hash(label))},${sql(translationStatus === 'reviewed' ? 'reviewed' : 'cross-checked')}) ON CONFLICT(id) DO UPDATE SET value_json=excluded.value_json,source_id=excluded.source_id,source_url=excluded.source_url,observed_at=excluded.observed_at,content_hash=excluded.content_hash,confidence=excluded.confidence;`); labelAssertions += 1;
    }
  }

  const manifest = {
    schemaVersion:3, release:'roman-catholic-pt-2026-v2', runId, generatedAt, sourceBuild:report.build, sourceBuildDigest:releaseDigest, publicationStatus, churchId:'roman-catholic', jurisdictionId:'pt', year:2026,
    expectedOccurrences:occurrences.length, expectedDays:365, expectedLabels:occurrences.length * PUBLIC_LOCALES.length, expectedCalendarAssertions:snlAssertions, expectedLabelAssertions:labelAssertions, publicLocales:PUBLIC_LOCALES, dropboxManifestPath, validationReportPath,
    sources:sources.map(({id,name,url,authority,usage}) => ({id,name,url,authority,usage})),
    provenancePolicy:{ portugalDateAuthority:'portugal-national-liturgy-secretariat', labelSourceIsNotCalendarIdentity:true, generalRomanAndRomcalNeverAssertTransferredPortugalDates:true, firstEventByCivilDateMatchingForbidden:true, portugalSpecificLocalizationMustBeReviewed:true, higherAuthorityLabelSourceUrlsPreserved:true },
    replacementPolicy:{ scope:'roman-catholic/pt/2026 only', stagingSliceReplacement:true, sharedCanonicalObservancesRetained:true },
    safety:{ automaticFutureProductionWrites:false, productionApproved:false, generatedFromExactApprovedOverlayOnly:true, descriptionsCopied:false, validationStatus:'cross-checked', d1ImportCompatible:true, stagingOnly:true },
  };
  return { sql:`${statements.join('\n')}\n`, manifest };
}

function main() {
  const inputPath = argument('--input'); const sqlPath = argument('--sql'); const manifestPath = argument('--manifest'); const dropboxManifestPath = argument('--dropbox-manifest-path'); const publicationStatus = argument('--publication-status','publishable');
  ...
  const report = JSON.parse(fs.readFileSync(path.resolve(inputPath),'utf8')); const result = buildPortugalD1ReleaseV2({ report, dropboxManifestPath, publicationStatus });
  fs.mkdirSync(path.dirname(path.resolve(sqlPath)), { recursive:true }); fs.mkdirSync(path.dirname(path.resolve(manifestPath)), { recursive:true }); fs.writeFileSync(path.resolve(sqlPath), result.sql, 'utf8'); fs.writeFileSync(path.resolve(manifestPath), `${JSON.stringify(result.manifest,null,2)}\n`, 'utf8');
