...
function reviewDecision(id, canonicalEventId, labels=null) { return { id, canonicalEventId, decision:'approved', ...(labels ? { labels } : {}) }; }
...
for (let index = 0; index < dates.length; index += 1) {
  ...
  if (dateISO === '2026-02-07') {
    id = 'FiveWoundsLord'; canonicalEventId = 'rc-pt:FiveWoundsLord'; name = 'As Cinco Chagas do Senhor'; rank = 'feast'; decisionId = 'pt-2026-five-wounds';
  } else if (dateISO === '2026-05-14') {
    id = 'StMatthias'; canonicalEventId = 'rc:StMatthias'; name = 'S. Matias, apóstolo'; rank = 'feast'; decisionId = 'pt-2026-ascension-transfer';
  } else if (dateISO === '2026-05-17') {
    id = 'Ascension'; canonicalEventId = 'rc:Ascension'; name = 'Ascension of the Lord'; rank = 'solemnity'; generalDate = '2026-05-14'; decisionId = 'pt-2026-ascension-transfer';
  } else if (dateISO === '2026-07-29') {
    id = 'StMartha'; canonicalEventId = 'rc:StMartha'; name = 'Santos Marta, Maria e Lázaro'; rank = 'memorial';
  } else if (dateISO === '2026-10-09') {
    id = 'StJohnHenryNewman'; canonicalEventId = 'rc:StJohnHenryNewman'; name = 'São João Henrique Newman, presbítero e Doutor da Igreja'; rank = 'optional-memorial';
  }

  items.push({
    id:`effective-${index}`, sourceOccurrenceId:`source-${index}`, sourceUid:`uid-${index}`, dateISO, canonicalEventId,
    category: canonicalEventId.includes('FiveWounds') ? 'feast' : 'saint', rank, labels:{ pt:name },
    ...
    resolution: decisionId ? 'approved-delta' : 'inherit-general', reviewStatus: decisionId ? 'approved' : 'inherited-safe', decisionId, publicationAllowed:false,
  });

  if (canonicalEventId.startsWith('rc:') && !['StJohnHenryNewman','StMatthias'].includes(id)) {
    const mirrorName = id === 'StMartha' ? 'Saint Martha' : name;
    mirrorBase.push(mirrorEvent(id, generalDate, mirrorName, rank));
    const romcalId = `romcal_${id}`;
    romcalEnglish.push({ id:romcalId, dateISO:generalDate, name:mirrorName, rank });
    romcalSpanish.push({ id:romcalId, dateISO:generalDate, name:`ES ${mirrorName}`, rank });
  }
}
...
for (let index = 0; index < 24; index += 1) {
  ...
  items.push({
    id:`effective-extra-${index}`, sourceOccurrenceId:`source-extra-${index}`, sourceUid:`uid-extra-${index}`, dateISO, canonicalEventId:`rc:${id}`,
    category:'saint', rank:'optional-memorial', labels:{ pt:name }, source:{ id:'portugal-national-liturgy-secretariat', sourceRecordHash:`extra-hash-${index}`, occurrenceAssertion:true },
    generalRomanBinding:{ generalRomanId:id, generalRomanDateISO:dateISO }, resolution:'inherit-general', reviewStatus:'inherited-safe', decisionId:null, publicationAllowed:false,
  });
...
const effective = { mode:'effective-portugal-calendar-preview', reviewPlanStatus:'approved-liturgical-decisions', publicationAllowed:true, productionWriteAllowed:false, summary:{ sourceOccurrences:389, uniqueDays:365, preparedDecisionsUsed:15 }, items };
...
const localizationReview = {
  schemaVersion:1, releaseScope:'roman-catholic-pt-2026-overlay-v2', reviewed:true, productionWriteAllowed:false,
  canonicalLabels:{ 'rc:StMatthias':{ en:'Saint Matthias, Apostle', es:'San Matías, apóstol', fr:'Saint Matthias, apôtre', it:'San Mattia, apostolo' } },
...
const inputs = { effective, review, localizationReview, mirrorPayloads:{ en:mirrorPayload('en'), fr:mirrorPayload('fr'), it:mirrorPayload('it') }, romcalPayload:{ packageVersion:'test', pinnedSourceCommit:'test', english:romcalEnglish, spanish:romcalSpanish }, year:2026, sourceCommit:'fixture' };
...
const martha = result.occurrences.find((item) => item.canonicalEventId === 'rc:StMartha');
assert.equal(martha.labels.en.label, 'Saints Martha, Mary and Lazarus');
assert.equal(martha.labels.es.label, 'Santos Marta, María y Lázaro');
assert.equal(martha.labels.fr.label, 'Saintes Marthe, Marie et saint Lazare');
assert.equal(martha.labels.it.label, 'Santi Marta, Maria e Lazzaro');
assert.ok(['en','es','fr','it'].every((locale) => martha.labels[locale].source === 'holy-see-calendar-authority'));
assert.ok(martha.labels.en.sourceUrl.includes('vatican.va'));
...
assert.equal(newman.labels.en.source, 'holy-see-calendar-authority');

const unsafe = structuredClone(effective); unsafe.productionWriteAllowed = true;
assert.throws(() => buildPortugalOverlayV2({ ...inputs, effective:unsafe }), /not approved and release-build eligible/u);
const unsafeLocalization = structuredClone(localizationReview); unsafeLocalization.safety.dateAuthority = true;
assert.throws(() => buildPortugalOverlayV2({ ...inputs, localizationReview:unsafeLocalization }), /label-only safety boundary/u);

console.log('Portugal v2 build preserves 389 observances, exact reviewed localization and higher-authority multilingual corrections with source provenance.');
