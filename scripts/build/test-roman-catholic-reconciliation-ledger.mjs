...
assert(ledger.counts.temporale === 114 && ledger.counts.temporalRules === 14 && ledger.counts.temporalFamilyMembers === 100, 'The ledger must distinguish fourteen TemporalRules from 100 approved TemporalRuleFamily members.');
...
assert(ledger.counts.sourceBound === 201 && ledger.counts.sourceBoundOccurrences === 205, 'The ledger must distinguish 201 source-bound days from 205 source-bound canonical occurrences.');
assert(ledger.counts.unresolved === 164, 'Every unreviewed official day must remain explicit and unresolved.');
...
assert(ledger.entries.find(item => item.dateISO === '2026-12-01')?.sourceBinding?.legacyObservanceId === 'rc:AdventWeekday1Tuesday', 'First Tuesday of Advent family binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-16')?.canonicalObservanceId === 'observance:advent-weekday-3-wednesday:roman-catholic', 'Reviewed Advent weekday boundary drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-17')?.canonicalObservanceId === 'observance:advent-weekday-december-17:roman-catholic', 'Late Advent December 17 canonical binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-23')?.sourceBinding?.legacyObservanceId === 'rc:AdventWeekdayDec23', 'Late Advent December 23 source binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-24')?.classification === 'unresolved', 'Portugal December 24 morning form must remain unresolved pending jurisdiction review.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-29')?.canonicalObservanceId === 'observance:christmas-octave-day-5:roman-catholic', 'Christmas Octave fifth-day canonical binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-31')?.sourceBinding?.legacyObservanceId === 'rc:ChristmasWeekdayDec31', 'Christmas Octave seventh-day source binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-01-18')?.canonicalObservanceId === 'observance:ordinary-time-sunday-2:roman-catholic', 'Second Sunday in Ordinary Time family binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-10-25')?.sourceBinding?.legacyObservanceId === 'rc:OrdSunday30', 'Thirtieth Sunday in Ordinary Time family binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-02-22')?.sourceBinding?.legacyObservanceId !== 'rc:OrdSunday7', 'Lent boundary must suppress the seventh Ordinary Time Sunday candidate.');
assert(ledger.entries.find(item => item.dateISO === '2026-11-01')?.sourceBinding?.legacyObservanceId !== 'rc:OrdSunday31', 'All Saints must suppress the thirty-first Ordinary Time Sunday candidate.');
assert(ledger.entries.find(item => item.dateISO === '2026-11-22')?.sourceBinding?.legacyObservanceId !== 'rc:OrdSunday34', 'Christ the King must suppress the thirty-fourth Ordinary Time Sunday candidate.');
...
assert(ledger.entries.find(item => item.dateISO === '2026-12-08')?.sourceBinding?.legacyObservanceId === 'rc:ImmaculateConception', 'Immaculate Conception must suppress the Advent-weekday candidate without losing its fixed Sanctorale binding.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-09')?.classification === 'unresolved', 'Suppressed Juan Diego Advent-weekday candidate must not count as a TemporalRuleFamily occurrence.');
...
const january20Options = ledger.entries.find(item => item.dateISO === '2026-01-20')?.fixedOptions ?? [];
assert(january20Options.length === 2 && january20Options.some(item => item.canonicalObservanceId === 'observance:fabian-pope:roman-catholic') && january20Options.some(item => item.canonicalObservanceId === 'observance:sebastian-martyr:roman-catholic'), '20 January must preserve Saint Fabian and Saint Sebastian as distinct fixed options.');
const april23Options = ledger.entries.find(item => item.dateISO === '2026-04-23')?.fixedOptions ?? [];
assert(april23Options.length === 2 && april23Options.some(item => item.canonicalObservanceId === 'observance:george-martyr:roman-catholic') && april23Options.some(item => item.canonicalObservanceId === 'observance:adalbert-prague:roman-catholic'), '23 April must preserve Saint George and Saint Adalbert as distinct fixed options.');
...
assert(ledger.entries.find(item => item.dateISO === '2026-08-10')?.canonicalObservanceId === 'observance:lawrence-rome:roman-catholic', 'Saint Lawrence canonical binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-12-26')?.canonicalObservanceId === 'observance:stephen-protomartyr:roman-catholic', 'Saint Stephen canonical binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-02-14')?.canonicalObservanceId === 'observance:cyril-methodius:roman-catholic', 'Saints Cyril and Methodius canonical binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-06-13')?.canonicalObservanceId === 'observance:anthony-lisbon:roman-catholic', 'Saint Anthony canonical binding drifted.');
assert(ledger.entries.find(item => item.dateISO === '2026-08-12')?.classification === 'unresolved', 'An unreviewed day must remain unresolved; text similarity must not create an identity binding.');
...
console.log('Portugal reconciliation ledger passed: 365/365 classified, 14 TemporalRules + 100 precedence-resolved TemporalRuleFamily members + 80 exact fixed Sanctorale occurrences + 11 movable/transfer bindings = 205 source-bound occurrences across 201 days, 164 explicit unresolved days and no label-derived identity.');
