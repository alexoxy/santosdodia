...
for (const day of occurrences) {
  ...
  statements.push(`INSERT INTO calendar_occurrences (id,import_run_id,church_id,jurisdiction_id,canonical_event_id,date_iso,native_calendar_system,native_year,rank_code,validation_status,publication_status,created_at,updated_at) VALUES (${sql(occurrenceId)},${sql(runId)},'roman-catholic','pt',${sql(eventId)},${sql(dateISO)},'gregorian',${year},${sql(cleanRank(day.primary?.grade))},'cross-checked',${sql(publicationStatus)},${sql(generatedAt)},${sql(generatedAt)}) ON CONFLICT DO UPDATE SET import_run_id=excluded.import_run_id,church_id=excluded.church_id,jurisdiction_id=excluded.jurisdiction_id,canonical_event_id=excluded.canonical_event_id,date_iso=excluded.date_iso,native_calendar_system=excluded.native_calendar_system,native_year=excluded.native_year,rank_code=excluded.rank_code,validation_status='cross-checked',publication_status=excluded.publication_status,updated_at=excluded.updated_at;`);
  const occurrenceIdSql = `(SELECT id FROM calendar_occurrences WHERE church_id='roman-catholic' AND jurisdiction_id='pt' AND canonical_event_id=${sql(eventId)} AND date_iso=${sql(dateISO)} LIMIT 1)`;
  statements.push(`DELETE FROM calendar_occurrence_assertions WHERE occurrence_id=${occurrenceIdSql} AND source_id IN ('litcal-api','portugal-national-liturgy-secretariat');`);
  statements.push(`DELETE FROM source_assertions WHERE subject_type='calendar-occurrence' AND subject_id=${occurrenceIdSql} AND field LIKE 'label.%' AND source_id IN ('litcal-api','portugal-national-liturgy-secretariat','romcal-general-roman-es');`);
  ...
  for (const [sourceId, sourceUrl, recordHash] of assertions) {
    ...
    statements.push(`INSERT INTO calendar_occurrence_assertions (id,occurrence_id,source_id,asserted_date_iso,source_record_url,source_record_hash,observed_at,validation_status) VALUES (${sql(assertionId)},${occurrenceIdSql},${sql(sourceId)},${sql(dateISO)},${sql(sourceUrl)},${sql(recordHash)},${sql(generatedAt)},'cross-checked') ON CONFLICT(id) DO UPDATE SET occurrence_id=excluded.occurrence_id,source_id=excluded.source_id,asserted_date_iso=excluded.asserted_date_iso,source_record_url=excluded.source_record_url,source_record_hash=excluded.source_record_hash,observed_at=excluded.observed_at,validation_status='cross-checked';`);
  }

  for (const locale of ['en', 'pt', 'es', 'fr', 'it']) {
    const label = String(day.labels?.[locale]?.label ?? '').normalize('NFC').trim();
    if (!label) throw new Error(`${dateISO} is missing ${locale} label.`);
    statements.push(`INSERT INTO calendar_occurrence_labels (occurrence_id,locale,name,description,translation_status,source_locale) VALUES (${occurrenceIdSql},${sql(locale)},${sql(label)},NULL,'source',${sql(locale)}) ON CONFLICT(occurrence_id,locale) DO UPDATE SET name=excluded.name,description=NULL,translation_status='source',source_locale=excluded.source_locale;`);
    const sourceId = sourceForLocale(locale);
    const claimId = hash(`${occurrenceId}|label.${locale}|${sourceId}|${label}`);
    statements.push(`INSERT INTO source_assertions (id,subject_type,subject_id,field,value_json,source_id,source_url,observed_at,content_hash,confidence) VALUES (${sql(claimId)},'calendar-occurrence',${occurrenceIdSql},${sql(`label.${locale}`)},${sql(JSON.stringify(label))},${sql(sourceId)},${sql(sources.find((item) => item.id === sourceId)?.url ?? '')},${sql(generatedAt)},${sql(hash(label))},'cross-checked') ON CONFLICT(id) DO UPDATE SET subject_id=excluded.subject_id,field=excluded.field,value_json=excluded.value_json,source_id=excluded.source_id,source_url=excluded.source_url,observed_at=excluded.observed_at,content_hash=excluded.content_hash,confidence=excluded.confidence;`);
  }
}
