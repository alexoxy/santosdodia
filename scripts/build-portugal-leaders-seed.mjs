...
function parseCsv(text) {
  ...
  return nonEmpty.map((values, rowIndex) => Object.fromEntries(headers.map((header, column) => [header, values[column] ?? '']))).map((item, index) => ({ ...item, _row: index + 2 }));
}
...
function values(rows) { return rows.map(tuple).join(',\n'); }
...
function buildSeed(rows, verifiedAt) {
  ...
  for (const [externalId, [id, name, level, parentId, officialUrl]] of Object.entries(JURISDICTIONS)) {
    jurisdictionRows.push([id, 'church:roman-catholic', parentId, level, name, 'PT', `PT-${slug(name).toUpperCase()}`, null, officialUrl, null, null, verifiedAt, verifiedAt]);
  }

  const people = [], names = [], externalIds = [], offices = [], assertions = [];
  for (const row of rows) {
    const canonicalPerson = personId(row.name);
    const externalId = row.person_id.split(':').at(-1);
    ...
    externalIds.push(['person', canonicalPerson, 'catholic-hierarchy', externalId, row.reference_source_url]);
    offices.push([officeId, canonicalPerson, jurisdictionId, type, row.office_title, appointedAt, null, null, status, verifiedAt, verifiedAt]);
    const assertionValue = JSON.stringify({ title: row.office_title, verificationStatus: row.verification_status, referenceUrl: row.reference_source_url, note: row.notes });
