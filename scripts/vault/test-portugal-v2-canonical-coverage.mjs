...
assert(explicitOccurrences.length === 63 && explicitOccurrences.length === coverage.coverage.explicitOccurrenceAnchors, 'Explicit canonical Occurrence count must remain exactly sixty-three.');
assert(explicitBridges.length === 63 && explicitBridges.length === coverage.coverage.explicitOccurrenceAnchors, 'Explicit legacy bridge count must equal the sixty-three canonical Occurrences.');
...
assert(explicitOccurrenceById.size === 63 && explicitBridgeByOccurrence.size === 63 && fixedMappingByOccurrence.size === 63, 'Fixed Sanctorale canonical/source identities must be unique.');
...
try {
  ...
  assert(coveredSourceIds.size === 115 && coveredSourceHashes.size === 115 && coveredOccurrenceIds.size === 115, 'Fixed, TemporalRule and TemporalRuleFamily source/canonical identities must not overlap.');
...
assert(totalMapped === 126 && totalMapped === coverage.coverage.mappedOccurrenceAnchors, 'Combined canonical shadow coverage must be exactly 126/389.');
assert(coverage.coverage.remainingLegacyOccurrences === 263 && coverage.coverage.remainingLegacyOccurrences === 389 - totalMapped, 'Remaining legacy count must be exactly 263.');
...
assert(coveragePercent === 32.391, `Unexpected canonical coverage percentage ${coveragePercent}.`);
console.log(`Portugal v2 canonical migration gate passed: ${totalMapped}/389 (${coveragePercent}%) = 63 exact fixed Sanctorale + 5 TemporalRule + 47 precedence-surviving family rows + 11 movable/transfer rows; 263 remaining, promotion blocked.`);
