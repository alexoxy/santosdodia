...
assert(explicitOccurrences.length === 80 && explicitOccurrences.length === coverage.coverage.explicitOccurrenceAnchors, 'Explicit canonical Occurrence count must remain exactly eighty.');
assert(explicitBridges.length === 80 && explicitBridges.length === coverage.coverage.explicitOccurrenceAnchors, 'Explicit legacy bridge count must equal the eighty canonical Occurrences.');
...
assert(Array.isArray(fixedSanctoraleShadow.mappings) && fixedSanctoraleShadow.mappings.length === 80, 'Fixed Sanctorale shadow must contain exactly eighty exact source bindings.');
const fixedSanctoraleMappingDigest = createHash('sha256').update(JSON.stringify(fixedSanctoraleShadow.mappings)).digest('hex');
assert(fixedSanctoraleMappingDigest === 'fcefa8b3d7c131945bf31c54212387ef1592dd2046f7fcf6fa3d3440c5aea4d0', `Fixed Sanctorale mapping set differs from the explicitly reviewed approved-artifact rows: ${fixedSanctoraleMappingDigest}.`);
...
assert(familyPresentLegacyIds.length === 100 && familyPresentSet.size === 100 && familyPresentLegacyIds.length === coverage.coverage.temporalRuleFamilyShadowOccurrences, 'Temporal family coverage must be exactly 100 unique precedence-surviving rows.');
assert(familyMappings.length === 100 && new Set(familyMappings.map((item) => item.sourceOccurrenceId)).size === 100 && new Set(familyMappings.map((item) => item.sourceRecordHash)).size === 100 && new Set(familyMappings.map((item) => item.occurrenceId)).size === 100, 'Temporal family coverage requires 100 unique exact source and canonical occurrence mappings.');
assert(familyMappings.every((item) => familyPresentSet.has(item.legacyObservanceId) && ['weekday', 'solemnity', 'celebration with precedence over solemnities'].includes(item.legacyRank) && item.reviewStatus === 'inherited-safe' && item.resolution === 'inherit-general-canonical-binding'), 'Temporal family exact mappings differ from the approved precedence-surviving identities.');
assert(Array.isArray(temporalFamilyShadow.suppressedCandidates) && temporalFamilyShadow.suppressedCandidates.length === 26, 'Temporal family snapshot must preserve exactly 26 precedence or season-boundary suppressions.');
...
assert(explicitOccurrenceById.size === 80 && explicitBridgeByOccurrence.size === 80 && fixedMappingByOccurrence.size === 80, 'Fixed Sanctorale canonical/source identities must be unique.');
...
try {
  ...
  assert(coveredSourceIds.size === 185 && coveredSourceHashes.size === 185 && coveredOccurrenceIds.size === 185, 'Fixed, TemporalRule and TemporalRuleFamily source/canonical identities must not overlap.');
...
assert(totalMapped === 196 && totalMapped === coverage.coverage.mappedOccurrenceAnchors, 'Combined canonical shadow coverage must be exactly 196/389.');
assert(coverage.coverage.remainingLegacyOccurrences === 193 && coverage.coverage.remainingLegacyOccurrences === 389 - totalMapped, 'Remaining legacy count must be exactly 193.');
...
assert(familyPresentSet.has('rc:MonHolyWeek') && familyPresentSet.has('rc:WedHolyWeek'), 'Holy Week source bindings are incomplete.');
assert(familyPresentSet.has('rc:MonOctaveEaster') && familyPresentSet.has('rc:SatOctaveEaster'), 'Easter Octave source bindings are incomplete.');

const policy = coverage.equivalencePolicy ?? {};
for (const key of [
  'exactLegacyObservanceIdBridgeRequired','exactDateRequired','rankCompatibilityRequired',
  'legacyMemorialMayRefineToObligatoryMemorialWhenOfficialSourceCodeIsMO','legacyOptionalMemorialRequiresOfficialSourceCodeMF','churchAndJurisdictionMustRemainFixed',
...
assert(coveragePercent === 50.386, `Unexpected canonical coverage percentage ${coveragePercent}.`);
console.log(`Portugal v2 canonical migration gate passed: ${totalMapped}/389 (${coveragePercent}%) = 80 exact fixed Sanctorale + 5 TemporalRule + 100 precedence-surviving family rows + 11 movable/transfer rows; 193 remaining, promotion blocked.`);
