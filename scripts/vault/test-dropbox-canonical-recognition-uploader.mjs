...
async function writeRelease(dataset, generatedAt) {
  ...
  const built = buildCanonicalRecognitionVaultRelease(dataset, personDataset, {
...
try {
  ...
  const initialized = await uploadCanonicalRecognitionRelease({
  ...
  assert.deepEqual(firstPointer.churches, ['eastern-orthodox', 'roman-catholic']);
  ...
  const repeated = await uploadCanonicalRecognitionRelease({
    inputRoot: first.directory,
    token: 'test-token',
    fetchImpl: dropbox.fetchImpl,
    promoteCurrent: true,
  ...
  const advanced = await uploadCanonicalRecognitionRelease({
  ...
  await assert.rejects(
    uploadCanonicalRecognitionRelease({
      inputRoot: second.directory,
      token: 'test-token',
      fetchImpl: dropbox.fetchImpl,
      promoteCurrent: true,
  ...
  console.log(`Canonical Recognition Dropbox uploader passed generic Vault isolation: ${first.built.manifest.rootSha256.slice(0, 12)} → ${second.built.manifest.rootSha256.slice(0, 12)}.`);
} finally {
