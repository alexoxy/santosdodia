...
export function planWikidataLabelRun({ config, identityManifest, identityReport, identityLedger, previousProgress = null } = {}) {
  if (config?.schemaVersion !== 1 || !supportedEnrichmentId(config?.enrichmentId) || config?.sourceId !== 'wikidata') throw new Error('Labels config has the wrong identity/schema.');
  if (identityManifest?.stage !== 'global-candidate-identity-ledger' || identityManifest?.mode !== 'staging' || identityManifest?.publish !== false) throw new Error('Labels enrichment requires the global staging identity ledger.');
  if (identityManifest.rootSha256 !== identityReport?.rootSha256 || identityReport?.freezeIdentityGateEligible !== true || identityReport?.identityConflictCount !== 0) throw new Error('Labels enrichment requires one conflict-free identity root.');
  if (!Array.isArray(identityLedger) || identityLedger.length !== identityReport.uniqueIdentityCount) throw new Error('Labels enrichment identity count mismatch.');
  ...
  if (!Array.isArray(config.locales) || config.locales.length !== 10) throw new Error('Labels enrichment must declare all 10 site locales.');
  if (config.policy?.exactQidInputOnly !== true || config.policy?.languageFallbacksForbidden !== true || config.policy?.automaticCanonicalNameSelection !== false || config.policy?.productionPublication !== false) throw new Error('Labels enrichment safety policy is incomplete.');
  if (config.enrichmentId === 'saints-labels-v3' && (config.policy?.translationForbidden !== true || config.policy?.sitelinkTitleEvidenceAllowed !== true || config.locales.some((locale) => !locale.wikipediaSite))) throw new Error('Labels v3 sitelink safety/configuration is incomplete.');
  for (const identity of identityLedger) if (!/^Q[1-9]\d*$/u.test(identity?.qid ?? '') || identity.entityId !== `wikidata:${identity.qid}` || identity.publish !== false) throw new Error('Labels enrichment received an unsafe identity record.');

  let start = 0;
  let successfulRuns = 0;
  let cumulativeEntitiesRequested = 0;
  if (previousProgress) {
    if (previousProgress.schemaVersion !== 1 || previousProgress.enrichmentId !== config.enrichmentId || previousProgress.sourceId !== config.sourceId) throw new Error('Previous labels progress has wrong identity/schema.');
    if (previousProgress.identityRootSha256 !== identityManifest.rootSha256) throw new Error('Identity root changed; labels progress must restart.');
    if (!Number.isSafeInteger(previousProgress.nextEntityOffset) || previousProgress.nextEntityOffset < 0 || previousProgress.nextEntityOffset > identityLedger.length) throw new Error('Previous labels cursor is invalid.');
  ...
  return {
    ...
    completed,
    shouldRun: !completed,
    reason: completed ? `${config.enrichmentId}-complete` : previousProgress ? `resume-${config.enrichmentId}` : `start-${config.enrichmentId}`
  };
}
