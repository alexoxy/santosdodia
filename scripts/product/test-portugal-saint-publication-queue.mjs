...
const calendar={build:'roman-catholic-pt-overlay-v2',productionWriteAllowed:false,calendarCoverage:{occurrences:7,coveredDays:1},productReadiness:{labelCount:35},occurrences:[
 ...
 occurrence('rc-pt:StJohnVianneyProper','feast','São João Maria Vianney','Saint John Mary Vianney'),
 occurrence('rc-pt:StsLocalMartyrs','feast','Santos Mártires locais','Local Saints and Martyrs'),
]};
...
assert.equal(result.items[5].classification.kind,'single-person-observance');
assert.equal(result.items[5].identityMatch.status,'unique-exact-candidate');
assert.equal(result.items[6].classification.kind,'collective-person-observance');
