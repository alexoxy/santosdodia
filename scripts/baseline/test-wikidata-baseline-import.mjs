...
const common = {
  queryVersion: 'recognition-v1', normalizationVersion: '1.1', reviewVersion: '1.1', importVersion: '1.0', entityLimit: 200,
...
assert.equal(first.entityLimit, 200);
assert.ok(first.reviewedStream.endsWith('/batch-000000'));
assert.ok(first.receiptStream.endsWith('/batch-000000/chunk-000000'));

function finalize(plan, { offset = plan.entityOffset, count = 200, sourceCount = 4008, sourceNextPage = 10, runId = 'run-a' } = {}) {
  const batch = {
...
const at200 = finalize(first);
assert.equal(at200.lastImported.completedSourceBatch, false);
assert.equal(at200.lastImported.nextEntityOffset, 200);
assert.equal(at200.cumulativeEntitiesImported, 200);
const second = planBaselineImport({ reviewedProgress, importProgress: at200, ...common });
assert.equal(second.targetStartPage, 0);
assert.equal(second.entityOffset, 200);
assert.equal(second.reason, 'continue-source-batch');
assert.ok(second.receiptStream.endsWith('/chunk-000200'));

const finalChunkPlan = { ...second, entityOffset: 4000, receiptStream: 'baseline-import-receipts/saints/v1/wikidata/recognition-v1/batch-000000/chunk-004000', previousProgress: { ...at200, cumulativeEntitiesImported: 4000 } };
...
assert.throws(() => planBaselineImport({ reviewedProgress, importProgress: { ...at200, d1ImportVersion: '9.9' }, ...common }), /pipeline version/u);
assert.throws(() => finalizeBaselineImport({
  ...
  d1Batch: { entityOffset: 0, entityCount: 201, nextEntityOffset: 201, sourceEntityCount: 4008, languageReviewVersion: '1.1', idempotencyKey: 'x', statementsSha256: 'x', sourceRunId: 'run-a' },
